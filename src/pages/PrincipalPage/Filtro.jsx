...
export const Filtros = () => {
  ...
  const handleProvinceSelection = async (selectedProvince) => {
    ...
    const changeFilter = { ...filter, provinces: selectedProvince, cities: "" };
  ...
  const cities = useCities(
    selectedProvince ? selectedProvince : "Escoge una provincia"
  );
  ...
  return (
    <div className=" flex flex-row justify-center items-center">
      <div className=" border h-[40px] border-solid border-gray-200 rounded-[80px] bg-white shadow-md flex flex-row items-center justify-between p-3 mq900:p-1  max-w-full  z-[3] ">
        <input
          className="hidden mq900:block font-medium ml-3 w-[150px] outline-none"
          ...
          <div className="fixed top-0 left-0 w-full h-full flex items-start justify-center z-50 bg-black bg-opacity-50">
            <div className="relative z-10 flex flex-col w-[400px] top-[108px] bg-white shadow-xl p-4 rounded-[20px] border border-solid border-gray-200">
              ...
              <input
                type="text"
                placeholder={
                  filter.provinces ? `${filter.provinces}` : "¿Dónde?"
                }
                value={searchProvinceText}
                onChange={handleChangeProvince}
              ...
              <div className="relative">
                ...
                >
                  {" "}
                  {filter.square ? `${filter.square}` : "¿Cuántas?"}
                </button>

                {opcionesAbiertas && (
                  <div className=" ml-8 z-10 mt-2 border border-solid border-gray-200 rounded-[20px] bg-white shadow-lg">
                    <div
                      className="flex w-[200px] px-4 py-2 text-sm text-gray-700  focus:outline-none "
                      role="menuitem"
                    >
                      <span className="mr-[55px]">Mascotas</span>
                      <div className="flex">
                        <button
                          className="flex items-center w-[20px] h-[20px] justify-center focus:outline-none rounded-[20px]"
                          onClick={decrementarCantidad}
                        >
                          -
                        </button>
                        <input
                          className="outline-none w-[35px] flex text-center"
                          value={filter.square}
                          readOnly
                        />
                        <button
        ...
        <button className="cursor-pointer  py-2 text-white  bg-chocolate-100 w-[42.5px] rounded-[80px] flex items-center justify-center px-[20px] hover:bg-chocolate-200">
          <box-icon name="search" size="25px" color="white"></box-icon>
        </button>
      </div>
      <div className="relative">
        <button
          className=" mq900:text-4px flex mq900:h-[30px] h-[40px] ml-2 font-custom font-medium items-center rounded-[80px] mq900:p-1 p-2 bg-white border border-gray-200 shadow-md hover:border-gray-900 "
        ...
        <a
          className="text-chocolate-100 text-sm hover:underline cursor-pointer flex justify-center"
          onClick={resetFilter}
        >
          Limpiar
        </a>
